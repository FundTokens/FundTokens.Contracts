...
├─ Inflow Thread 1 ─┐
├─ Inflow Thread 2  │
├─ Inflow Thread N  │─→ Enables parallel transactions
│                   │
├─ Outflow Thread 1─┤
├─ Outflow Thread 2 │
├─ Outflow Thread N─┘
...
│                │ Valid - ascending
│
└─ Validation: Deployed contracts check ordering
```
...
└─ Format:
   ...
   └─ Example (2 assets):
      ...
      ├─ 1 asset:   88 bytes
      ├─ 2 assets: 128 bytes ← One broadcast NFT
      └─ 3+ assets: 168 bytes ← Two broadcast NFT
...
└─ Format:
   ...
   ├─ [32:40 bytes] → Fee amount (int64 LE)
